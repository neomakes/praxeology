...
class Heartbeat:
    ...
    def lightweight_check(self) -> dict:
        ...
        try:
            ...
            row = conn.execute(
                "SELECT COUNT(*) AS cnt FROM tactical WHERE status = 'pending'"
            ).fetchone()
    ...
    def heavyweight_trigger(self, reasons: list) -> dict:
        ...
        try:
            ...
            overdue_work = conn.execute(
                "SELECT COUNT(*) FROM tactical WHERE tier = 'work' AND status = 'pending'"
            ...
            if priority_score >= 5:
                ...
                existing = conn.execute(
                    "SELECT id FROM tactical WHERE title LIKE '[Heartbeat]%' AND status = 'pending'"
                ).fetchone()
                if existing is None:
                    conn.execute(
                        "INSERT INTO tactical (tier, title, description, status, priority) VALUES (?, ?, ?, ?, ?)",
